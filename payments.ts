...
function calculateAmount(
  ...
  winnings?: number
): { amount: number; winnings?: number } {
  if (paymentType === "vip_weekly") {
    return { amount: VIP_WEEKLY_AMOUNT };
  }
  if (paymentType === "pay_after") {
    ...
    return {
      amount: parseFloat((winnings * PAY_AFTER_RATE).toFixed(2)),
      winnings,
    };
  }
...
function validateInput(input: CreatePaymentInput) {
  ...
  if (!["vip_weekly", "pay_after"].includes(input.paymentType))
    throw new Error("Invalid payment type.");
}
...
export async function createPayment(input: CreatePaymentInput) {
  ...
  const { amount, winnings } = calculateAmount(
    input.paymentType,
    input.winnings
  );
  ...
  if (existing) {
    throw new Error("This MoMo reference has already been submitted.");
  }
